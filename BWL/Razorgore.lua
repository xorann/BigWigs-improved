...
L:RegisterTranslations("enUS", function() return {
    ...
    volley_cmd = "volley",
    volley_name = "Fireball Volley",
    volley_desc = "Warn for Fireball Volley",
    volley_trigger = "Fireball Volley",
...
BigWigsRazorgore.toggleoptions = { "engage", "volley", "mc", "eggs", "phase", "bosskill" }
...
function BigWigsRazorgore:OnEnable()
    ...
    self:RegisterEvent("CHAT_MSG_SPELL_CREATURE_VS_PARTY_DAMAGE", "Volley")
    self:RegisterEvent("CHAT_MSG_SPELL_CREATURE_VS_SELF_DAMAGE", "Volley")
...
function BigWigsRazorgore:Volley(arg1)
    if string.find(arg1, L["volley_trigger"]) and self.db.profile.volley then
         self:TriggerEvent("BigWigs_StartBar", self, L["volley_bar"], 10, "Interface\\Icons\\Spell_Fire_SelfDestruct")
    end
end
...
function BigWigsRazorgore:BigWigs_RecvSync(sync, rest)
	if sync == self:GetEngageSync() and rest and rest == boss and not started then
		...
		if self.db.profile.engage then
            ...
            self:TriggerEvent("BigWigs_StartBar", self, L["engage_bar"], 35, "Interface\\Icons\\Spell_Holy_PrayerOfHealing")
            self:ScheduleEvent("countdown5", "BigWigs_Message", 30, "", "Urgent", true, "Five")
            self:ScheduleEvent("countdown4", "BigWigs_Message", 31, "", "Urgent", true, "Four")
            self:ScheduleEvent("countdown3", "BigWigs_Message", 32, "", "Urgent", true, "Three")
            self:ScheduleEvent("countdown2", "BigWigs_Message", 33, "", "Urgent", true, "Two")
            self:ScheduleEvent("countdown1", "BigWigs_Message", 34, "", "Urgent", true, "One")
            
            -- 08 25(17) 38(13) 54(16) 09(15)
		end
    end
