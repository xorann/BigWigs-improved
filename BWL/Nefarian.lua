...
L:RegisterTranslations("enUS", function() return {
    ...
    landing_trigger = "Nefarian 's Shadow Flame",
...
function BigWigsNefarian:OnEnable()
    ...
    self:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE", "LandingEvent")
	self:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_FRIENDLYPLAYER_DAMAGE", "LandingEvent")
	self:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_PARTY_DAMAGE", "LandingEvent")
...
function BigWigsNefarian:LandingEvent(msg)
    if string.find(msg, L["landing_trigger"]) then 
        DEFAULT_CHAT_FRAME:AddMessage("BigWigs Debug: landing trigger");    
    end
end
...
function BigWigsNefarian:BigWigs_RecvSync( sync )    
	...
	elseif sync == "NefarianFear" and self.db.profile.fear then
		...
		self:ScheduleEvent("BigWigs_Message", 24, L["fear_soon_warning"], "Important")
		self:TriggerEvent("BigWigs_StartBar", self, L["fear_bar"], 29, "Interface\\Icons\\Spell_Shadow_PsychicScream")
	end
end
